...
def create_camera(payload: dict, db: Session = Depends(get_db)):
    ...
    return {"id": camera.id, "name": camera.name, "source_type": camera.source_type,
            "mode": camera.mode, "is_active": camera.is_active}
...
def list_cameras(db: Session = Depends(get_db)):
    ...
    for c in cameras:
        ...
        result.append({
            ...
            "mode": c.mode, "is_active": c.is_active, "zones": zones_data
        })
    return result
...
def get_camera(camera_id: str, db: Session = Depends(get_db)):
    ...
    return {"id": cam.id, "name": cam.name, "source_type": cam.source_type,
            "mode": cam.mode, "is_active": cam.is_active}
...
def create_zone(camera_id: str, payload: dict, db: Session = Depends(get_db)):
    ...
    zone = Zone(
        ...
        loitering_time_threshold=payload.get("loitering_time_threshold", 30),
    )
    db.add(zone)
    db.commit()
    db.refresh(zone)
